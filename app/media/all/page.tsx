...
export const metadata: Metadata = {
  ...
  description: "WITH by NovolBaのすべての記事一覧。インタビュー・対談・イベントレポートなど。",
};
  ...
  return (
    <main className="bg-white">
      ...
      >
        ...
        <h1 className="text-4xl font-bold tracking-widest text-gray-800">すべての記事</h1>
        <div className="mt-4 w-12 h-0.5" style={{ backgroundColor: "#3dbdac" }} />
        <p className="mt-3 text-sm text-gray-400">{allArticles.length}件</p>
      </section>
      ...
      <section className="py-16 px-6">
        <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-12">
                    ...
                    <li key={article.id}>
                      ...
                      >
                        <div className="shrink-0 w-40 h-28 sm:w-52 sm:h-36 relative rounded-lg overflow-hidden bg-gray-100">
                          {thumb ? (
                            <Image src={thumb} alt={article.title} fill className="object-cover" sizes="208px" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-xl" style={{ backgroundColor: "#e6f7f5" }}>📝</div>
                          )}
                        </div>
                        <div className="flex flex-col gap-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {categoryName && (
                              <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: "#e6f7f5", color: "#3dbdac" }}>
                                {categoryName}
                              </span>
                            )}
                          </div>
          ...
          <aside className="lg:w-64 shrink-0 flex flex-col gap-8">
            <div>
              <h3 className="text-sm font-bold text-gray-700 tracking-widest mb-4 pb-2 border-b border-gray-200">カテゴリー</h3>
              <ul className="flex flex-col gap-2">
                {CATEGORY_ORDER.map((cat) => (
                  <li key={cat}>
                    <Link href={`/media/category/${CATEGORY_SLUG_MAP[cat]}/`} className="flex items-center gap-2 text-sm text-gray-600 hover:text-teal-500 transition-colors">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-700 tracking-widest mb-4 pb-2 border-b border-gray-200">最新の投稿</h3>
              <ul className="flex flex-col gap-4">
                {latestArticles.map((article) => {
                  const thumb = article.eyecatch?.url ?? extractFirstImage(article.content);
                  return (
                    <li key={article.id}>
                      <Link href={`/media/${article.slug ?? article.id}/`} className="flex gap-3 group hover:opacity-80 transition-opacity">
            ...
            <div className="pt-4 border-t border-gray-100">
              <Link href="/media" className="text-sm font-medium hover:opacity-70 transition-opacity" style={{ color: "#3dbdac" }}>
                ← メディアトップへ戻る
              </Link>
            </div>
          </aside>
