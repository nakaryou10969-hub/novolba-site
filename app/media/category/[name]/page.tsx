...
export async function generateStaticParams() {
  return Object.keys(SLUG_TO_CATEGORY).map((slug) => ({ name: slug }));
}
...
export default async function MediaCategoryPage({ params }: Props) {
  ...
  const categoryArticles = allArticles.filter((a) => getCategoryName(a) === categoryName);
      ...
      <section className="py-12 px-6">
        <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-12">
                      ...
                      <Link href={getMediaArticlePath(article)} className="flex gap-5 group hover:opacity-80 transition-opacity">
                        <div className="shrink-0 w-32 h-24 sm:w-48 sm:h-32 relative rounded-lg overflow-hidden bg-gray-100">
                          {thumb ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-2xl" style={{ backgroundColor: "#e6f7f5" }}>📝</div>
                          )}
                        </div>
                        <div className="flex flex-col gap-2 min-w-0">
                          <span className="text-xs font-bold tracking-widest uppercase" style={{ color: "#3dbdac" }}>{categoryName}</span>
                          <h2 className="text-base sm:text-lg font-bold text-gray-800 leading-snug group-hover:underline">{article.title}</h2>
          ...
          <aside className="lg:w-64 shrink-0 flex flex-col gap-8">
            <div>
              <h3 className="text-sm font-bold text-white px-3 py-2 rounded mb-4" style={{ backgroundColor: "#3dbdac" }}>カテゴリー</h3>
              <div className="flex flex-wrap gap-2">
                {CATEGORY_ORDER.map((cat) => (
                  <Link
                    key={cat}
                    href={`/media/category/${CATEGORY_TO_SLUG[cat]}/`}
                    className={`text-sm px-3 py-1 rounded hover:opacity-80 transition-opacity ${cat === categoryName ? "text-white font-bold" : "text-gray-600 bg-gray-100"}`}
            ...
            <div>
              <h3 className="text-sm font-bold text-white px-3 py-2 rounded mb-4" style={{ backgroundColor: "#3dbdac" }}>最新の投稿</h3>
