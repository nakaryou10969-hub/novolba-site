...
function BeforeAfterSlider() {
  const slides = [
    { src: "/bofore.png", alt: "入居前のオフィス" },
    { src: "/after.png",  alt: "入居後のオフィス" },
  ];
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      {/* 画像エリア */}
      <div className="relative w-full sm:w-3/4 lg:w-1/2 overflow-hidden rounded-2xl shadow-md">
        <div
          className="flex transition-transform duration-500 ease-in-out"
          style={{ transform: `translateX(-${current * 100}%)` }}
        >
          {slides.map((slide) => (
            <div key={slide.src} className="min-w-full relative aspect-[4/3]">
              <Image
                src={slide.src}
                alt={slide.alt}
                fill
        ...
        <button
          onClick={prev}
          aria-label="前の画像"
          className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full w-9 h-9 flex items-center justify-center shadow transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#3dbdac" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
        ...
        <button
          onClick={next}
          aria-label="次の画像"
          className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full w-9 h-9 flex items-center justify-center shadow transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#3dbdac" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
      ...
      <div className="flex gap-2">
        {slides.map((_, i) => (
          <button
            ...
            style={{ backgroundColor: i === current ? "#3dbdac" : "#d1d5db" }}
          />
        ))}
      </div>
    </div>
...
export default function BasicOfficeClient() {
  return (
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-16 rounded-3xl border-2 p-8" style={{ borderColor: "#3dbdac", backgroundColor: "#f0fdfb" }}>
            ...
            <div className="flex justify-center">
              <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-100 w-full sm:w-[70%] lg:w-1/2">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-base">
              ...
              <tbody>
                {pricing.map((row, i) => (
                  <tr key={row.members} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
